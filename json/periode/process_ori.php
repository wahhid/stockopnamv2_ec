...
function stockbins($periode, $storagebin){
...
function transstockdetail($articleno, $batchno, $periode, $periodesession, $biname){
    if($periodesession == 1){
        ...
        if(isset($batchno)){
            $query .= " AND a.bin='" . $biname . "' and a.batchno='" . $batchno . "'";
        }else{
            $query .= " AND a.bin='" . $biname . "'  and a.batchno is null";
        }
    ...
    if($periode['status'] == 1){
        ...
        if($resultstockbins['success']){        
            ...
            for($i=0;$i<count($arrstockbin);$i++){
                ...
                $resulttransstockdetail = transstockdetail($stockbin['articleno'],$stockbin['batchno'],$periodeid,$periode['periodesession'],$stockbin['storagebin']);
...
//                }
			...
			$query1 = mysql_query("SELECT * from stockbin where periode ='" . $_SESSION['periode'] . "'");
				...
				if ($fisik == $opnam)
					{
					$query = "UPDATE stockbin SET deff=(stocksap) WHERE id=" . $id2;
					$rs = mysql_query($query);    

					}
				elseif ($fisik < $opnam) 
					{
					$query = "UPDATE stockbin SET deff=((stocksap) - (countedqty)) WHERE id=" . $id2;
					$rs = mysql_query($query);    
